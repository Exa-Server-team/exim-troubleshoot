...
# =============================================================================
# SECTION 8 — EMAIL DELIVERY TRACE (INTERACTIVE, MULTI-FILTER)
# =============================================================================
hdr "8. Email Delivery Trace"
sep

# ── Helper: search across live log + all rotated/gzipped archives ─────────────
#
#   search_exim_logs <grep_args...>
#   Searches /var/log/exim_mainlog, exim_mainlog.1, and all exim_mainlog.*.gz
#   Prints results with source filename prefix, sorted by timestamp.
#
search_exim_logs() {
  local RESULTS=""
  ...
  if [[ -f /var/log/exim_mainlog ]]; then
    local LIVE
    LIVE=$(grep -h "$@" /var/log/exim_mainlog 2>/dev/null)
    [[ -n "$LIVE" ]] && RESULTS+="$LIVE"$'\n'
  fi

  # Rotated plain-text log (exim_mainlog.1)
  if [[ -f /var/log/exim_mainlog.1 ]]; then
    local ROT1
    ROT1=$(grep -h "$@" /var/log/exim_mainlog.1 2>/dev/null)
    [[ -n "$ROT1" ]] && RESULTS+="$ROT1"$'\n'
  fi

  # All gzip-compressed archives
  local GZ_FILES
  GZ_FILES=$(ls /var/log/exim_mainlog*.gz 2>/dev/null | sort -V)
  if [[ -n "$GZ_FILES" ]]; then
    while IFS= read -r GZ; do
      local GZ_HIT
      GZ_HIT=$(zgrep -h "$@" "$GZ" 2>/dev/null)
      [[ -n "$GZ_HIT" ]] && RESULTS+="$GZ_HIT"$'\n'
    done <<< "$GZ_FILES"
  fi

  # Output — already in roughly date order since Exim logs are chronological
  printf '%s' "$RESULTS"
}

# ── Count available log files ─────────────────────────────────────────────────
echo ""
LIVE_LOG_EXISTS=false
[[ -f /var/log/exim_mainlog ]] && LIVE_LOG_EXISTS=true

GZ_COUNT=$(ls /var/log/exim_mainlog*.gz 2>/dev/null | wc -l)
ROT1_EXISTS=false
[[ -f /var/log/exim_mainlog.1 ]] && ROT1_EXISTS=true

info "Available Exim log sources:"
$LIVE_LOG_EXISTS && pass "  /var/log/exim_mainlog         (live)" \
                 || warn "  /var/log/exim_mainlog         (not found)"
$ROT1_EXISTS    && pass "  /var/log/exim_mainlog.1       (rotated plain)" \
                 || info "  /var/log/exim_mainlog.1       (not found)"
if [[ $GZ_COUNT -gt 0 ]]; then
  pass "  exim_mainlog.*.gz             ($GZ_COUNT archive file(s) found)"
  info "  Archive range:"
  ls /var/log/exim_mainlog*.gz 2>/dev/null | sort -V | \
    awk 'NR==1{first=$0} END{last=$0; printf "    Oldest: %s\n    Newest: %s\n", first, last}'
else
  warn "  No .gz archives found in /var/log/"
fi

TOTAL_SOURCES=0
$LIVE_LOG_EXISTS && ((TOTAL_SOURCES++))
$ROT1_EXISTS     && ((TOTAL_SOURCES++))
((TOTAL_SOURCES += GZ_COUNT))
info "Total log sources to search: $TOTAL_SOURCES"

# ── Search filter menu ────────────────────────────────────────────────────────
echo ""
echo -e "  ${BOLD}Search filters (leave blank to skip that filter):${RESET}"
echo ""

read -r -p "  Sender address / domain    : " F_SENDER
read -r -p "  Recipient address / domain : " F_RECIPIENT
read -r -p "  Subject keyword            : " F_SUBJECT
read -r -p "  Message-ID                 : " F_MSGID
read -r -p "  Remote IP / hostname       : " F_HOST
read -r -p "  Date filter (e.g. 2024-01) : " F_DATE
read -r -p "  Max results to display     : " F_LIMIT
read -r -p "  Show only FAILED/deferred? [y/N]: " F_FAIL_ONLY

# Default limit
F_LIMIT="${F_LIMIT:-200}"

# Validate at least one filter given
if [[ -z "$F_SENDER" && -z "$F_RECIPIENT" && -z "$F_SUBJECT" && \
      -z "$F_MSGID"  && -z "$F_HOST"      && -z "$F_DATE" ]]; then
  warn "No filters entered — skipping trace. Re-run and enter at least one filter."
else
  echo ""
  info "Building search across all $TOTAL_SOURCES log source(s)..."
  sep

  # ── Stage 1: Collect message-IDs from sender / recipient / host / date ──────
  #
  #  Exim log format (one line per event):
  #  YYYY-MM-DD HH:MM:SS <msgid> [flags] from=<addr> / to=<addr> / host=... / ...
  #
  #  Strategy:
  #    Pass 1 — narrow to lines matching every supplied filter via chained greps.
  #    Extract the Exim message-IDs from those lines.
  #    Pass 2 — fetch ALL log lines for those message-IDs (so we get the full
  #              delivery chain: arrival, routing, completion/failure).

  echo ""
  info "Pass 1 — Applying filters to locate matching message-IDs..."
  echo ""

  # Build grep pipeline dynamically
  # We start with all log content, then pipe through grep for each filter
  GREP_PIPELINE="search_exim_logs -E '.*'"

  # We'll use a function to run the chained search cleanly
  run_filtered_search() {
    # Start from all log content
    local STREAM
    STREAM=$(search_exim_logs -l "" 2>/dev/null; \
             # Fallback: just cat all sources manually since -l won't work cross-archive
             true)

    # Actually: pipe full log content through successive greps
    {
      # Live log
      [[ -f /var/log/exim_mainlog   ]] && cat /var/log/exim_mainlog
      [[ -f /var/log/exim_mainlog.1 ]] && cat /var/log/exim_mainlog.1
      # Archives
      ls /var/log/exim_mainlog*.gz 2>/dev/null | sort -V | \
        while read -r gz; do zcat "$gz" 2>/dev/null; done
    } | {
      # Date filter first (most selective, cheapest)
      if [[ -n "$F_DATE" ]]; then
        grep -F "$F_DATE"
      else
        cat
      ...
      # Fail/deferred filter
      if [[ "${F_FAIL_ONLY,,}" == "y" ]]; then
        grep -iE "failed|defer|DEFER|rejected|bounce|unroutable"
      else
        cat
      fi
    }
  ...
  # Run the filter pipeline and capture matching lines
  MATCHED_LINES=$(run_filtered_search 2>/dev/null)
  MATCHED_COUNT=$(echo "$MATCHED_LINES" | grep -c '.' || echo 0)

  info "Pass 1 matched $MATCHED_COUNT log line(s)"

  if [[ $MATCHED_COUNT -eq 0 ]]; then
    warn "No log entries matched the supplied filters across all log sources."
    warn "Tips:"
    echo "    • Check spelling — Exim logs addresses in lowercase"
    echo "    • Try a partial string (e.g. just the domain, not full address)"
    echo "    • Widen the date filter or remove it entirely"
    echo "    • Logs older than the oldest .gz may have been purged"
  else

    # ── Extract Exim Message-IDs from matched lines ──────────────────────────
    # Exim message IDs appear as the 3rd whitespace-delimited field after timestamp+time
    # Format: YYYY-MM-DD HH:MM:SS <msgid> ...
    # msgid is a 16-char alphanumeric like: 1qABCD-0001XY-Z3
    MSGIDS=$(echo "$MATCHED_LINES" | \
      awk '{print $3}' | \
      grep -E '^[A-Za-z0-9]{6}-[A-Za-z0-9]{6}-[A-Za-z0-9]{2}$' | \
      sort -u)

    MSGID_COUNT=$(echo "$MSGIDS" | grep -c '.' || echo 0)
    info "Pass 1 identified $MSGID_COUNT unique Exim message-ID(s)"

    # ── Stage 2: Subject matching via exim -Mvh (if subject filter given) ────
    SUBJECT_MATCHED_IDS=""
    if [[ -n "$F_SUBJECT" && -n "$MSGIDS" ]]; then
      echo ""
      info "Pass 1.5 — Checking message subjects (requires each queued/spool msgid)..."
      while IFS= read -r MID; do
        [[ -z "$MID" ]] && continue
        SUBJ=$(exim -Mvh "$MID" 2>/dev/null | grep -i "^Subject:" | head -1)
        if echo "$SUBJ" | grep -qi "$F_SUBJECT"; then
          SUBJECT_MATCHED_IDS+="$MID"$'\n'
        fi
      done <<< "$MSGIDS"

      SUB_COUNT=$(echo "$SUBJECT_MATCHED_IDS" | grep -c '.' || echo 0)

      if [[ $SUB_COUNT -gt 0 ]]; then
        pass "Subject filter matched $SUB_COUNT message-ID(s) still in queue/spool"
        # Narrow msgids to subject matches if we got any
        MSGIDS="$SUBJECT_MATCHED_IDS"
        MSGID_COUNT=$SUB_COUNT
      else
        warn "Subject filter '$F_SUBJECT' not found in current spool headers"
        info "(Subject is not stored in mainlog; only queued messages can be checked by subject)"
        info "Proceeding with all $MSGID_COUNT matched message-IDs from Pass 1"
      fi
    fi

    # ── Stage 2: Full delivery chain fetch for each message-ID ───────────────
    echo ""
    info "Pass 2 — Fetching complete delivery chain for $MSGID_COUNT message-ID(s)..."
    sep
    ...
    if [[ -n "$MSGIDS" ]]; then
      while IFS= read -r MID; do
        [[ -z "$MID" ]] && continue
        # Fetch ALL lines for this msgid across all log sources
        MID_LINES=$(
          {
            [[ -f /var/log/exim_mainlog   ]] && cat /var/log/exim_mainlog
            [[ -f /var/log/exim_mainlog.1 ]] && cat /var/log/exim_mainlog.1
        ...
        if [[ -n "$MID_LINES" ]]; then
          FULL_CHAIN_OUTPUT+="$(echo "$MID_LINES" | head -50)"$'\n'
        fi
      done <<< "$MSGIDS"
    else
      # No clean msgids extracted — fall back to showing the raw matched lines
      FULL_CHAIN_OUTPUT="$MATCHED_LINES"
    fi

    # ── Output results ────────────────────────────────────────────────────────
    CHAIN_LINE_COUNT=$(echo "$FULL_CHAIN_OUTPUT" | grep -c '.' || echo 0)
    echo ""
    info "Full delivery chain — $CHAIN_LINE_COUNT line(s) | showing up to $F_LIMIT"
    sep
    echo ""
    echo "$FULL_CHAIN_OUTPUT" | grep '.' | head -"$F_LIMIT" | sed 's/^/    /'
    echo ""

    if [[ $CHAIN_LINE_COUNT -gt $F_LIMIT ]]; then
      warn "Output truncated to $F_LIMIT lines. Increase limit or narrow filters."
    fi

    # ── Per message-ID summary ────────────────────────────────────────────────
    if [[ -n "$MSGIDS" && $MSGID_COUNT -le 20 ]]; then
      echo ""
      sep
      info "Per-message delivery summary:"
      echo ""
      while IFS= read -r MID; do
        [[ -z "$MID" ]] && continue
        printf "  ${BOLD}%-22s${RESET}" "$MID"
        ...
        # Extract key fields
        FROM=$(echo "$MID_ALL" | grep -oP '(?<=<= )[^ ]+|(?<=from )<[^>]+>' | head -1)
        TO=$(echo "$MID_ALL"   | grep -oP '(?<= => )[^ ]+|(?<=for )<[^>]+>' | head -1)
        STATUS=""
        if echo "$MID_ALL" | grep -q "Completed"; then
          STATUS="${GREEN}Delivered${RESET}"
        elif echo "$MID_ALL" | grep -qi "frozen"; then
          STATUS="${RED}Frozen${RESET}"
        elif echo "$MID_ALL" | grep -qi "failed\|rejected"; then
          STATUS="${RED}Failed${RESET}"
        elif echo "$MID_ALL" | grep -qi "defer"; then
          STATUS="${YELLOW}Deferred${RESET}"
        else
          STATUS="${CYAN}In progress${RESET}"
        fi

        ARRIVE=$(echo "$MID_ALL" | head -1 | awk '{print $1, $2}')
        echo -e "  From: ${FROM:-?}  To: ${TO:-?}  Status: $STATUS  Arrived: $ARRIVE"
      done <<< "$MSGIDS"
    fi

    # ── Exim spool inspection (if message still in queue) ─────────────────────
    echo ""
    sep
    info "Spool/queue inspection for matched message-IDs:"
    echo ""
    QUEUED_FOUND=0
    if [[ -n "$MSGIDS" ]]; then
      while IFS= read -r MID; do
        [[ -z "$MID" ]] && continue
        if exim -Mvh "$MID" &>/dev/null 2>&1; then
          ((QUEUED_FOUND++))
          echo ""
          warn "Message $MID is still in the Exim queue:"
          echo ""
          info "  Headers:"
          exim -Mvh "$MID" 2>/dev/null | grep -iE "^(From|To|Subject|Date|Message-ID|Return-Path|X-)" | \
            sed 's/^/      /'
          echo ""
          info "  Body preview (first 10 lines):"
          exim -Mvb "$MID" 2>/dev/null | head -10 | sed 's/^/      /'
          echo ""
          info "  Transport info:"
          exim -Mvl "$MID" 2>/dev/null | sed 's/^/      /'
        fi
      done <<< "$MSGIDS"
    fi
    [[ $QUEUED_FOUND -eq 0 ]] && info "  No matched messages currently in queue (already delivered or removed)"

    # ── Statistics summary ────────────────────────────────────────────────────
    echo ""
    sep
    info "Search statistics:"
    echo "    Filter — Sender    : ${F_SENDER:-  (any)}"
    echo "    Filter — Recipient : ${F_RECIPIENT:-  (any)}"
    echo "    Filter — Subject   : ${F_SUBJECT:-  (any — spool-only check)}"
    echo "    Filter — Msg-ID    : ${F_MSGID:-  (any)}"
    echo "    Filter — Host/IP   : ${F_HOST:-  (any)}"
    echo "    Filter — Date      : ${F_DATE:-  (any)}"
    echo "    Failed/deferred only : ${F_FAIL_ONLY:-N}"
    echo "    Log sources searched : $TOTAL_SOURCES"
    echo "    Pass 1 raw matches   : $MATCHED_COUNT"
    echo "    Unique message-IDs   : $MSGID_COUNT"
    echo "    Full chain lines     : $CHAIN_LINE_COUNT"
    sep
  fi

  # ── Mailbox check ─────────────────────────────────────────────────────────
  if [[ -n "$F_RECIPIENT" && "$F_RECIPIENT" == *@* ]]; then
    echo ""
    MAIL_USER=$(echo "$F_RECIPIENT" | cut -d@ -f1)
    MAIL_DOMAIN=$(echo "$F_RECIPIENT" | cut -d@ -f2)
    info "Mailbox check for $F_RECIPIENT:"
    MAILBOX_PATH="/home/$MAIL_USER/mail/$MAIL_DOMAIN/$MAIL_USER"
    ALT_PATH=$(find /home -name "cur" -path "*$MAIL_DOMAIN*$MAIL_USER*" 2>/dev/null | head -1)
    if [[ -d "$MAILBOX_PATH" ]]; then
      MAILBOX_SIZE=$(du -sh "$MAILBOX_PATH" 2>/dev/null | cut -f1)
      pass "Mailbox found: $MAILBOX_PATH (size: $MAILBOX_SIZE)"
    elif [[ -n "$ALT_PATH" ]]; then
      pass "Mailbox found at: $ALT_PATH"
    else
      warn "Mailbox not found locally — may be remote or user does not exist"
    fi

    # Exim routing test
    echo ""
    info "Exim routing test for $F_RECIPIENT:"
    exim -bt "$F_RECIPIENT" 2>/dev/null | sed 's/^/    /' || \
      warn "Could not run Exim routing test (exim -bt)"

    # WHM MX check
    if [[ -x /usr/local/cpanel/bin/mxcheck ]]; then
      echo ""
      info "WHM MX check for domain $MAIL_DOMAIN:"
      /usr/local/cpanel/bin/mxcheck "$MAIL_DOMAIN" 2>/dev/null | sed 's/^/    /'
    fi
  fi
fi

# =============================================================================
# SECTION 9 — COMMON ISSUE CHECKLIST
# =============================================================================
...
# =============================================================================
# SECTION 10 — USEFUL REFERENCE LINKS
# =============================================================================
...
echo "    3. Use Section 8 trace to investigate specific addresses"
echo "    4. Check blacklist URLs in Section 10 for IP reputation"
